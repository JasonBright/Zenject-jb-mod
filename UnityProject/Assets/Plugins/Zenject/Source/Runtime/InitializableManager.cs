    ...
    public class InitializableManager
    {
            ...
            List<ValuePair<Type, Type>> dependencies)
        {
            ...
            for (int i = 0; i < initializables.Count; i++)
            {
                ...
                var dep = dependencies.Where( x => initializable.GetType().DerivesFromOrEqual( x.First ) ).Select( x => x.Second )
                    .ToArray();
        ...
        private void CheckPriorities(int priority, Type[] dependencies)
        {
            foreach (Type dependency in dependencies)
            {
                ...
                if(dependencyPriority > priority)
                    throw new Exception("Dependency priority must be less than or equal to the initializable priority");
            }
        }
        ...
        public void Add(IInitializable initializable, IAsyncInitializable asyncInitializalbe, int priority, Type[] dependencies)
        {
        ...
        private async UniTaskVoid InitAsync()
        {
            ...
            for (var index = 0; index < _initializables.Count; index++)
            {
                ...
                try
                {
                    if (waitForEndLayer && (initializable.Priority > repeatLayer) || (index == _initializables.Count - 1 && running.Count > 0))
                    {
        ...
        class InitializableInfo
        {
            ...
            public InitializableInfo(IInitializable initializable, IAsyncInitializable asyncInitializable, int priority, Type[] dependencies)
            {
